...
interface HeaderProps {
  ...
  activeTab: "voice_sentinel" | "pqc_shield" | "shor_lab" | "circuit_studio" | "bitcoin_arena" | "solana_relayer" | "anna_executa";
  setActiveTab: (tab: "voice_sentinel" | "pqc_shield" | "shor_lab" | "circuit_studio" | "bitcoin_arena" | "solana_relayer" | "anna_executa") => void;
...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header id="main-header" className="border-b border-slate-800 bg-slate-950/80 backdrop-blur-md sticky top-0 z-40">
      ...
      <div className="bg-gradient-to-r from-cyan-950/60 via-purple-950/60 to-emerald-950/60 border-b border-slate-800/80 px-4 py-1.5 text-xs text-slate-300 flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-cyan-500/20 text-cyan-300 border border-cyan-500/30">
            CALL-E VOICE RUNTIME ACTIVE
          </span>
          <span>
            Autonomous Emergency Phone Dispatcher & NIST FIPS 204 Lattice Cryptography
          </span>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex flex-wrap items-center justify-between gap-4">
        {/* Brand */}
        <div className="flex items-center gap-3 cursor-pointer" onClick={() => setActiveTab("voice_sentinel")}>
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-lg font-bold text-white tracking-tight">OmniCall Quantum Sentinel</h1>
            ...
            <p className="text-xs text-slate-400">
              Autonomous Voice Incident Dispatcher & NIST PQC Defense
            </p>
          </div>
        </div>
        ...
        <nav className="flex items-center p-1 bg-slate-900/90 border border-slate-800 rounded-xl text-xs font-medium">
          <button
            id="tab-voice-sentinel"
            onClick={() => setActiveTab("voice_sentinel")}
            className={`px-3 py-1.5 rounded-lg transition-all flex items-center gap-1.5 ${
              activeTab === "voice_sentinel"
                ? "bg-gradient-to-r from-red-600 via-rose-600 to-amber-600 text-white shadow-md shadow-rose-600/30 font-semibold"
                : "text-rose-300 hover:text-white"
            }`}
          >
            <Sparkles className="w-3.5 h-3.5 text-amber-300 animate-spin" />
            <span>Voice Sentinel (CALL-E)</span>
          </button>
          <button
            id="tab-shor-lab"
