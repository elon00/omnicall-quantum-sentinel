...
export function signPqcTransaction(
  ...
  const sigBytes = ml_dsa65.sign(msgBytes, secBytes);
...
export function runNistSelfTest(): {
  ...
  try {
    ...
    const sig = ml_dsa65.sign(testMsg, dsaKeys.secretKey);
    const dsaPassed = ml_dsa65.verify(sig, testMsg, dsaKeys.publicKey);

    // 3. Test Tamper Resistance (Fail-Closed)
    const tamperedSig = new Uint8Array(sig);
    tamperedSig[0] ^= 0xff; // Invert first byte
    const tamperRejected = !ml_dsa65.verify(tamperedSig, testMsg, dsaKeys.publicKey);
